public class Braid {
    ...
    public String enc(String pt, String k) throws InterruptedException{
        ...
        b1.start(pt2, k2, 'e'); // First block
        d1.join(); // First block
        b1.join(); // First block

        d2.start(b1.o, k1, 'd'); // Second block
        b2.start(d1.o, k2, 'd'); // Second block
        d2.join(); // Second block
        b2.join(); // Second block

        d3.start(b2.o, k1, 'e'); // Third block
        b3.start(d2.o, k2, 'e'); // Third block
        d3.join(); // Third block
        b3.join(); // Third block
    ...
    public String dec(String ct, String k) throws InterruptedException{
        ...
        b1.start(ct2, k2, 'd'); // First block
        d1.join(); // First block
        b1.join(); // First block

        d2.start(b1.o, k1, 'e'); // Second block
        b2.start(d1.o, k2, 'e'); // Second block
        d2.join(); // Second block
        b2.join(); // Second block

        d3.start(b2.o, k1, 'd'); // Third block
        b3.start(d2.o, k2, 'd'); // Third block
        d3.join(); // Third block
        b3.join(); // Third block
